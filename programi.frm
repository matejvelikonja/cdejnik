...
Begin VB.Form programi 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "CDejnik 3.0 (Programi)"
   ClientHeight    =   9510
   ClientLeft      =   1695
   ClientTop       =   1275
   ClientWidth     =   13305
   Icon            =   "programi.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin MSDBGrid.DBGrid seznamigre 
      Bindings        =   "programi.frx":0442
      Height          =   6855
      Left            =   5760
      OleObjectBlob   =   "programi.frx":0456
      TabIndex        =   0
      Top             =   240
      Width           =   7455
   End
...
Private Sub status()
If progi.Recordset.EOF = False And progi.Recordset.BOF = False Then
    trenutna = progi.Recordset.AbsolutePosition

    progi.Recordset.MoveLast
    With vrstica
        .Panels(1).Text = "Št. zapisov: " & progi.Recordset.RecordCount
        .Panels(2).Text = "Ste na zapisu št.: " & (trenutna + 1)
        .Panels(3).Text = "Ura je " & Time
        .Panels(4).Text = "Danes smo " & Date
    End With
    
    progi.Recordset.AbsolutePosition = trenutna
End If
End Sub
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    If progi.Recordset.EOF = True Then
        If KeyCode = vbKeyUp Then progi.Recordset.MovePrevious
        If KeyCode = vbKeyDown Then progi.Recordset.MoveFirst
        
    ElseIf progi.Recordset.BOF = True Then
        If KeyCode = vbKeyDown Then progi.Recordset.MoveNext
        If KeyCode = vbKeyUp Then progi.Recordset.MoveLast
        
    ElseIf progi.Recordset.EOF = False And progi.Recordset.BOF = False Then
        If KeyCode = vbKeyUp Then
            progi.Recordset.MovePrevious
        ElseIf KeyCode = vbKeyDown Then
            progi.Recordset.MoveNext
        End If
    End If
End Sub
...
' sposojeno da ne
     If progi.Recordset.EOF = False And progi.Recordset.BOF = False Then
            If progi.Recordset!sposojevalec <> "" Then
                sposojeno.Text = "Da"
                vrni.Enabled = True
        ...
        timer 'gre na timer
        vrstica.Panels(2).Text = "Ste na zapisu št.: " & (progi.Recordset.AbsolutePosition + 1)
        bar.Value = ((progi.Recordset.AbsolutePosition + 1) / progi.Recordset.RecordCount) * 100
     Else
...
If progi.Recordset.EOF = False And progi.Recordset.BOF = False Then
'išèi
    x = progi.Recordset.AbsolutePosition
    If Text1.Text = "" Then
    ...
    End If
        ...
        progi.Recordset.MoveFirst
        Do Until progi.Recordset.EOF = True
            If UCase(progi.Recordset!ime) Like "*" & niz & "*" Then
                MsgBox "Najden vnos " & progi.Recordset!ime, vbInformation
                Exit Sub
            Else
                progi.Recordset.MoveNext
            End If
        ...
        If MsgBox("Nobenega vnosa ni bilo najdenega.", vbInformation) = vbOK Then
            progi.Recordset.AbsolutePosition = x
            Exit Sub
        End If
Else
...
Private Sub izbrisi_Click()
    If progi.Recordset.EOF = True Or progi.Recordset.BOF = True Then
            MsgBox "Izberite vnos!", vbCritical
    Else
            If MsgBox("Ste preprièani, da želite izbrisati " & progi.Recordset!ime & "?", vbExclamation + vbYesNo + vbDefaultButton1) = vbYes Then
                pot = progi.Recordset!slika
                If (Dir(pot) <> "") Then
                    Kill (pot)
                End If
                With progi.Recordset
                .Delete
                .MoveLast
                End With
            Else
...
Private Sub popravig_Click()
    ...
    Else
        popravi.Show modal
        popravi.Caption = "Popravi vnos: " & progi.Recordset!ime
        popravi.preverip.Caption = "progi"
        Me.Enabled = False
    End If
End Sub
...
Private Sub sposodi_Click()
   ...
   If progi.Recordset.EOF = True Or progi.Recordset.BOF = True Then
    ...
    Else
        posodio.Show modal
        posodio.Caption = "Posodi: " & progi.Recordset!ime
        posodio.preveri.Caption = "progi"
        Me.Enabled = False
    End If
End Sub
...
Private Sub Timer2_Timer()
    If progi.Recordset.EOF = False And progi.Recordset.BOF = False Then 'veljaven zapis
    
        slikca = progi.Recordset!slika 'definicija
        nislike = App.Path & "\Pics\nopic.jpg"
            ...
            Else
                    ...
                    Else
                    lep.Text = progi.Recordset!ime
                    kontrola
                    y = App.Path & "\Pics\" & progi.Recordset!id & lep.Text & ".jpg"
                        If Dir(y) <> "" Then
                            With progi.Recordset
                            .Edit
                                !slika = y
                            ...
                            End With
                            slika.Picture = LoadPicture(progi.Recordset!slika)
                         Else
...
If progi.Recordset.EOF = True And progi.Recordset.BOF = True Then
...
Else
    ...
    Else
            With natisni
                .Show modal
                .Data1.DatabaseName = progi.DatabaseName
                .Data1.Refresh
                .Data1.RecordSource = "Select Ime, ID, Zvrst , St_CDejev From Programi order by ime"
                .Data1.Refresh
                .x.Caption = "progi"
            End With
            Me.Enabled = False
...
Private Sub vnos_Click()
    'prikaže okno vnos
    nov.Show modal
    nov.Caption = "Dodaj nov program"
    Me.Enabled = False
End Sub

Private Sub vrni_Click()
x = progi.Recordset!ime
With progi.Recordset
        .Edit
            !sposojevalec = Null
            !sposojeno = Null
